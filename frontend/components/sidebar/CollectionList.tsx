...
export function CollectionList({ collections, activeId, onSelect }: CollectionListProps) {
  const allCollection = collections.find((collection) => collection.id === "all");
  const groupedCollections = collections
    .filter((collection) => collection.id !== "all")
    .reduce<Record<string, Collection[]>>((acc, collection) => {
  ...
  return (
    <ul className="flex flex-col gap-0.5" role="listbox" aria-label="Collections">
      {allCollection ? renderCollection(allCollection) : null}
      {Object.entries(groupedCollections).map(([section, sectionCollections]) => (
        <li key={section} className="pt-2">
          <p className="px-3 pb-1 text-[11px] font-semibold uppercase" style={{ color: "var(--text-faint)" }}>
            {section}
          </p>
          <ul className="flex flex-col gap-0.5">
            {sectionCollections.map(renderCollection)}
          </ul>
        </li>
      ))}
    </ul>
