...
async def init_collections(client: AsyncQdrantClient) -> None:
    for profile_name, profile in list_document_embedding_profiles().items():
        ...
        for field_name, field_schema in (
            ...
            ("audience_roles", PayloadSchemaType.KEYWORD),
            ("tool_name", PayloadSchemaType.KEYWORD),
            ("quality", PayloadSchemaType.KEYWORD),
    ...
    points = [
        PointStruct(
            ...
            payload={
                ...
                "tool_url": chunk.tool_url,
                "short_description": chunk.short_description,
                "department": chunk.department,
                "primary_role": chunk.primary_role,
                "audience_roles": chunk.audience_roles,
                "importance_note": chunk.importance_note,
                "impact_note": chunk.impact_note,
                "rating": chunk.rating,
            },
        )
        ...
        results.extend(
            {
                ...
                "record_kind": hit.payload.get("record_kind", "document"),
                "bbox": hit.payload.get("bbox", []),
                "source_path": hit.payload.get("source_path", ""),
                "tool_name": hit.payload.get("tool_name", ""),
                "tool_url": hit.payload.get("tool_url", ""),
                "short_description": hit.payload.get("short_description", ""),
                "department": hit.payload.get("department", ""),
                "primary_role": hit.payload.get("primary_role", ""),
                "audience_roles": hit.payload.get("audience_roles", []),
                "importance_note": hit.payload.get("importance_note", ""),
                "impact_note": hit.payload.get("impact_note", ""),
